...
public class Director extends Empleado {
	...
	public void run() {
		...
		System.out.println("inicio atencion Director: " +call.getId()+"    "+ call.getFrom() + "   "+call.getId()+" running");
		int nlduracionllamada = duracionLlamada();
		try {
			Thread.sleep(nlduracionllamada*1000);			
			System.out.println("llamada procesada Director: " +call.getId()+"    "+ call.getFrom() + "   "+call.getId()+" runned 		duracion llamada(seg): "+nlduracionllamada+"  numerollamadasProcesadas:"+llamadasProcesadas);
			llamadasProcesadas++;
